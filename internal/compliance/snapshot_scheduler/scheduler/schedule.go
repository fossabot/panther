...
func PollAndIssueNewScans() error {
	...
	var integrationsToScan []*snapshotapimodels.SourceIntegrationMetadata
...
func getEnabledIntegrations() (integrations []*snapshotapimodels.SourceIntegration, err error) {
	err = genericapi.Invoke(
		lambdaClient,
		snapshotAPIFunctionName,
		&snapshotapimodels.LambdaInput{ListIntegrations: &snapshotapimodels.ListIntegrationsInput{
...
func scanIsStuck(integration *snapshotapimodels.SourceIntegration) bool {
	...
	return *integration.ScanStatus == snapshotapimodels.StatusScanning && scanIntervalElapsed(integration)
}

// scanIsNotOngoing checks if an integration's snapshot is currently running.
func scanIsNotOngoing(integration *snapshotapimodels.SourceIntegration) bool {
	if integration.SourceIntegrationStatus == nil {
		return true
	}

	return *integration.ScanStatus != snapshotapimodels.StatusScanning
}

// scanIntervalElapsed determines if a new scan needs to be started based on the configured interval.
func scanIntervalElapsed(integration *snapshotapimodels.SourceIntegration) bool {
